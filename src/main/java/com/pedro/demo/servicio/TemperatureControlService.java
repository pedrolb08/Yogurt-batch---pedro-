...
public class TemperatureControlService {
        ...
        new Thread(() -> {
            try {
                ...
                while (currentTemp < targetTemp) {
                    ...
                    recordTemperature(batch, currentTemp, TemperaturaLog.LogType.HEATING);
        ...
        new Thread(() -> {
            try {
                ...
                for (int i = 0; i < durationMinutes; i++) {
                    ...
                    recordTemperature(batch, temp, TemperaturaLog.LogType.HEATING);
    ...
    private void startCoolingProcess(YogurtBatch batch) {
        log.info("Starting cooling process for batch: {} to inoculation temperature: {}°C", 
            batch.getBatchCode(), batch.getRecipe().getInnoculationTemperature());
        ...
        new Thread(() -> {
            try {
                ...
                double targetTemp = batch.getRecipe().getInnoculationTemperature();
                ...
                while (currentTemp > targetTemp) {
                    ...
                    recordTemperature(batch, currentTemp, TemperaturaLog.LogType.COOLING);
    ...
    public void startIncubationControl(YogurtBatch batch) {
        ...
        new Thread(() -> {
            try {
                // Usar maxIncubationTime de la receta para calcular el tiempo de fin de incubación
                int maxIncubationMinutes = batch.getRecipe().getMaxIncubationTime();
                LocalDateTime endTime = LocalDateTime.now().plusMinutes(maxIncubationMinutes);
                ...
                while (LocalDateTime.now().isBefore(endTime)) {
                    ...
                    recordTemperature(batch, temp, TemperaturaLog.LogType.INCUBATION);
    ...
    private void recordTemperature(YogurtBatch batch, Double temperature, TemperaturaLog.LogType type) {
        TemperaturaLog tempLog = TemperaturaLog.builder()
            .batch(batch)
        ...
        temperaturaLogRepository.save(tempLog);
    }
    
    public Double getCurrentTemperature(Long batchId) {
        return temperaturaLogRepository.findByBatchAndTypeOrderByRecordedAtDesc(
            batchId, TemperaturaLog.LogType.INCUBATION)
            .stream()
            .findFirst()
            .map(TemperaturaLog::getTemperature)
            .orElse(null);
    }
}